...
def save_results(results: list[dict], output_dir: str = "outputs") -> str:
    ...
    for r in results:
        ...
        politician = r.get("politician", "unknown").replace(" ", "_")
        trade_type = r.get("trade_type", "unknown")
        fname = f"{politician}_{ticker}_{trade_type}.md"
        with open(individual_dir / fname, "w") as f:
            f.write(f"# {r['politician']} — {r['trade_type']} {r['ticker']}\n\n")
            f.write(f"**Date:** {r.get('trade_date', 'N/A')}\n")
            f.write(f"**Amount:** {r.get('amount_range', 'N/A')}\n")
            f.write(f"**Grade:** {r.get('grade', '?')}\n")
            f.write(f"**Suspicion Score:** {r.get('suspicion_score', 'N/A')}/5\n\n")
            f.write("---\n\n")
    ...
    with open(summary_path, "w") as f:
        ...
        suspicious = sorted(
            [r for r in results if r.get("suspicion_score") is not None],
            key=lambda x: x["suspicion_score"],
            reverse=True,
        )
        for r in suspicious[:5]:
            f.write(f"- **{r['politician']}** {r['trade_type']} {r['ticker']} "
                    f"({r['trade_date']}) — Suspicion: {r['suspicion_score']}/5, "
                    f"Grade: {r['grade']}\n")

        f.write("\n## Worst Trades\n\n")
        grade_order = {"F": 0, "D": 1, "D+": 1.3, "D-": 0.7, "C": 2, "C-": 1.7,
                       "C+": 2.3, "B": 3, "B-": 2.7, "B+": 3.3, "A": 4, "A-": 3.7, "A+": 4.3}
        ...
        f.write("\n## All Trades\n\n")
        f.write("| Politician | Trade | Ticker | Date | Grade | Suspicion |\n")
        f.write("|---|---|---|---|---|---|\n")
        for r in results:
            f.write(f"| {r['politician']} | {r['trade_type']} | {r['ticker']} "
                    f"| {r['trade_date']} | {r['grade']} | {r.get('suspicion_score', '?')}/5 |\n")
